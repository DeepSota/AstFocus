...
from model_wrapper.vid_model_top_k import C3D_K_Model, SLOWONLY_K_Model, \
    TSM_K_Model, C3D_K_Model_k400
import numpy as np
from attack.attackZERO import untargeted_video_attack
...
def main(args):
        ...
        if model_name == 'slowonly':
            vid_model = SLOWONLY_K_Model(model)
        if model_name == 'tsm':
        ...
        def GetPairs_ori( idx):
            ...
            # x0 = image_to_vector(model_name, x0)  # x0在[0,1]之间
            if x0.size(0) == 3:
                x0 = x0.transpose(1, 0)
            return x0.cuda(), x0
        ...
        for idx in range(0,  len(attacked_ids)):
            ...
            res, iter_num, adv_vid = untargeted_video_attack(vid_model, vid, x0, label, args)
            NUM += 1
            '--------------------complete-----------------------'
