...
class AST_S(nn.Module):
    ...
    def one_step_act(self, img, feature, feature_pre, resample, training=True):
        key, HX = self.focuser(img=img, feature=feature, feature_pre=feature_pre,  resample = resample, training=training)
        return key, HX
...
class Focuser(nn.Module):
    ...
    def forward(self, *argv, **kwargs):
        action_s, hx= self.policy.select_action(kwargs['feature'], kwargs['feature_pre'], self.memory, kwargs['resample'], kwargs['training'])
...
class PatchSampler(nn.Module):
    ...
    def random_sample(self, imgs, size):
        key_random = []
        for img in imgs:
            key_random.append(random_crop(img, size))
        return key_random
