...
def reward_advantage(logits,label,pre_confidence,pre_hunxiao_confidence):
...
def rep_frame(seq, actions, ignore_far_sim=True, temp_dist_thre=3, use_gpu=False):
    ...
    dist_mat = dist_mat + dist_mat.t()
    dist_mat = dist_mat[:, pick_idxs]
    dist_mat = dist_mat.min(0, keepdim=True)[0]
...
def attackreward(res,iter_num,vid,adv_vid):
    ...
    if (res):
        if (iter_num>15000):
            P = pertubation(vid,adv_vid)
            R = 0.999*torch.exp(-(P/0.05))
            P = P.cpu()
        else:
            P = pertubation(vid,adv_vid)
            R = torch.exp(-(P/0.05))
