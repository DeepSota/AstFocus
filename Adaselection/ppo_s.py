...
class Actor(nn.Module):
    def __init__(self, args, action_dim, hidden_state_dim=512,  ckpt=None, max_steps=3, tot_frames=16, use_stop=False):
        ...
        # self.AE= SelfAttention(1,1024,1024, 0.5)
        self.patch_size=66
        self.ckpt = ckpt
        self.tot_frames = tot_frames
        self.max_steps = max_steps
        self.embedding_size = 1280  # 2688 #2304  #2048
        self.global_embedding_size = 512

        self.hidden_size = 512  # 2048 for fcvid, 1024 for anet
        ...
        global_proj_kwargs = {
            'input_dim': self.global_embedding_size,  # + self.global_embedding_size,
        ...
        hidden_proj_kwargs = {
            'input_dim': self.hidden_size,  # + self.global_embedding_size,
            'hidden_dims': (self.small_hidden_size,),
            'use_batchnorm': False,
            'dropout': 0,
        }
    ...
    def re_act(self,vid, features,  global_fea_ori,GetFeatures):
        ...
        for focus_time_step in range(vid.size(0)):
            ...
            feature_pre = feature_pre.contiguous().cuda()  # contiguous()什么意思
            feature_pre = self.sub_proj_mlp(feature_pre)
            feature_cur = feature_cur_ori.contiguous().cuda()  # contiguous()什么意思
            feature_cur = self.proj_mlp(feature_cur).view(1, -1)
            state_obs_s = torch.cat([feature_cur, feature_pre.view(1, -1)], 1)
            # state_obs_s = self.AE(state_obs_s)
...
class Critic(nn.Module):
    def __init__(self, args, action_dim, hidden_state_dim=512,  ckpt=None, max_steps=3, tot_frames=16, use_stop=False):
        ...
        self.hidden_size = 1024  # 2048 for fcvid, 1024 for anet
...
class MAPPO(nn.Module):
    ...
    def update(self, memory, vid, features,   global_fea,  obs_share_t, GetFeatures,XX):
        ...
        # 做一步马尔可夫链
        # for reward in reversed(memory.rewards):
        #     rewards = torch.tensor(reward).cuda()
