...
def agent_output(agent,features):
    ...
    pidx = np.argsort(t_probs)[::-1].tolist()    # 由小到大排序
...
def get_patch(images, action_sequence, patch_size):
    ...
    patch_coordinate = torch.floor(action_sequence * (image_size - patch_size)).int()  # patch_coordinate（1,2）， action_sequence（0.333,0.667）
    key = []
    for i in range(batchsize):
      ...
      data = (x1, x2, y1, y2)
      key.append(data)  # key 只能append 一个元素，四个元素 可以改成一个list
...
def accuracy(output, label, topk=(1,)):
    ...
    pred = pred.t()  # 转置
    correct = pred.eq(label.view(1, -1).expand_as(pred))  # x.eq(y) x与y是否相等
...
def get_variable(inputs, cuda=False, **kwargs):
    if type(inputs) in [list, np.ndarray]: # 如果type 的类型是 list 或者 np.ndarray
...
class feature_extractor(nn.Module):
    ...
    def forward(self, vid):
        ...
        # print('conv5',res_conv5.size())
        res_pool5 = self.pool5(res_conv5)
        # print('pool',res_pool5.size())
        res_pool5 = res_pool5.view(res_pool5.size(0), -1)
        print('res_pool5',res_pool5.size())
        return res_pool5
