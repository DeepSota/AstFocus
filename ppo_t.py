...
class ActorCritic(nn.Module):
    ...
    def act(self, feature, global_fea , hx, memory_t, restart_batch=False, training=True):
        ...
        feature = feature.contiguous().cuda()  # contiguous()什么意思
        ...
        prob_t = self.actor_temporal(h_next)

        #print(prob_t)

        dist_t = Categorical(prob_t)
    ...
    def evaluate(self, state, hx, action):
        seq_l = state.size(0)
        batch_size=1
        h_t = init_hidden(seq_l, 512)
        c_t = init_hidden(seq_l, 512)
        state = state.view(seq_l,  -1)
        state, hidden = self.wdrnn(state, hx)
        state = state.view(seq_l, -1)

        action_probs = self.actor_temporal(state)

        dist = Categorical(action_probs * batch_size)
