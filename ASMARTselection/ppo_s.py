...
from ASMARTselection.modules import build_mlp
...
class ActorCritic(nn.Module):
    ...
    def act(self, state_ini, feature_pre, memory, use_pre, restart_batch=False, training=True):
        if restart_batch:
            del memory.hidden[:]
            batch_size = 1
        ...
        if use_pre:
            ...
            pre_state = self.proj_mlp(feature_pre)
            state =torch.cat([pre_state, state], 1)
            state =self.hidden_mlp(state)
...
class PPO_s(nn.Module):
    ...
    def select_action(self, state, feature_pre, memory, use_pre, restart_batch=False, training=True):
        return self.policy_old.act(state, feature_pre, memory, use_pre, restart_batch, training)
