...
from ASMARTselection.Reward import reward_advantage, \
    target_reward_advantage, rep_frame
...
def nes_frame(model, vid, prev_grads, actions, n, sigma, label, sub_num, xent=False, momentum=0.7):
    with torch.no_grad():
        len_frame = len(actions)
        grads = torch.zeros((len_frame, 3, 112, 112), device='cuda')
        batch_loss = []
        batch_noise = []
        batch_idx = []
        ...
        for _ in range(n // sub_num):
            ...
            mask_list_t = actions.flatten()
            MASK = torch.zeros(adv_vid_rs.size())
            for i in range(len_frame):
                key = mask_list_t[i]
                MASK[:,  key, :, :, :] = all_noise[:, i, :, :, :]
            ...
            ground_preds = F.softmax(logits.detach(), 1).view(sub_num, -1)[:, label]
            ...
            if (xent):
                loss = -torch.log(ground_preds)
            else:
                loss = torch.log(hunxiao_probs + 1e-10) - torch.log(ground_preds + 1e-10)
        ...
        loss_order = torch.zeros(batch_loss.size(0), device='cuda')
        sort_index = torch.sort(batch_loss, 0)[1]
        loss_order[sort_index.flatten()] = torch.arange(0, batch_noise.size(0), device='cuda',
                                                        dtype=torch.float)
        rank_weight = (loss_order / len(batch_loss)).cuda()
        grads += torch.sum(batch_noise / sigma * (batch_loss.view((-1,) + (1,) * (len(batch_noise.size()) - 1))), 0)
        return grads
...
def nes_target(model, vid, patch_size, mask_list, actions_t, n, ori_class, target_class, sub_num, w1, sigma=1e-3):
    with torch.no_grad():
...
def nes_patch(model, vid, patch_size, mask_list,  n,  target, rank_transform, sub_num,   sigma=1e-3):
    with torch.no_grad():
        grads = torch.zeros(16, 3, patch_size, patch_size, device='cuda')
        count_in = 0
        loss_total = 0
        batch_loss = []
        batch_noise = []
        ...
        assert n % sub_num == 0 and sub_num % 2 == 0
        for _ in range(n // sub_num):
            adv_vid_rs = vid.repeat((sub_num,) + (1,) * len(vid.size())).clone()
            noise_list = torch.randn((sub_num // 2,) + grads.size(), device='cuda') * sigma
            all_noise = torch.cat([noise_list, -noise_list], 0)
            MASK = torch.zeros(adv_vid_rs.size())
            b = mask_list.numpy()
            ...
            for x1, x2, y1, y2 in b:
                MASK[:, i, :, y1:y2, x1:x2] = all_noise[:, i, :, :, :]
                i += 1
...
def  untargeted_video_attack(vid_model, vid, x0, ori_class,  args,  eps=0.05,
    ...
    while num_iter < max_iter:
        ...
        for focus_time_step in range(vid.size(0)):
            ...
            key_s, HX_s = AST_s.one_step_act(img, feature_s, features, resample=True, training=True)
            ks = key_s.cpu().numpy()
        ...
        while True:
            ...
            for focus_time_step in range(vid.size(0)):
                ...
                if key_t:
                   ...
                   R_rep = rep_frame(feature_t, focus_time_step).cpu()
                else:
                   feature_pre = torch.ones(1, 1280)
                   R_rep = 0

                AST_t.focuser.memory.rewards.append(R_rep-R_rep)
...
def  targeted_video_attack( vid_model, vid, x0, ori_class, target_label, model_name, args,  eps=0.05,
                        max_lr=0.03, min_lr= 2.4e-3, effect_num=8, starting_eps=1, sample_per_draw=60,  sub_num_sample=2, max_iter=30000):
    ...
    while num_iter < max_iter:
        ...
        if num_iter > max_iter*1/3:
            if top_val.item() > target_prob.item()*6:
                return False, num_iter, adv_vid
        if num_iter > max_iter*1/2:
            if top_val.item() > target_prob.item() * 4:
                return False, num_iter, adv_vid
        if num_iter > max_iter*2/3:
            if top_val.item() > target_prob.item() * 2:
                return False, num_iter, adv_vid
        #---------------------------------------------------agent更新---------------------------------------------------
        # patch_agent更新
