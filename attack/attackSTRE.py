...
from Adaselection.utils import agent_output, finelist, sparse_perturbation, process_grad, speed_up_process
from Adaselection.utils_ada import init_hidden
from Adaselection.Reward import rep_frame, div_frame
from Utils.utils import pertubation
    ...
    while num_iter < max_iter:
        ...
        while True:
            ...
            if  ll >= 2 and ll <= len_up_band :
                break
            if step > 10:
                actions_t, key_t = speed_up_process(len_frame, len_limit)
                NONRE = False
                break
...
class Tattack_base(object):
    def __init__(self, model,   vid, tar_vid, ori_label, target_label, model_name,
        ...
        self.y0 = ori_label
        self.targeted = target_label.view(-1,1)
